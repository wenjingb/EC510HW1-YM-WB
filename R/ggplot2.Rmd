...
There are five variables in the dataframe, including species, petal width and length as well as sepal length and width. The Figure 1 tries to visualize the relationship between different iris species and their sepal length.
```{r fig1}
# Plot code here
plot(iris$Species, iris$Sepal.Length, xlab="Species", ylab="Sepal Length", title ("Iris Species and Their Sepal Length"))
```
...
```{r fig2}
# Plot code here
plot(iris$Sepal.Length, iris$Petal.Length, xlab="Petal Length", ylab="Sepal Length", title("Sepal Length vs. Petal Length"), col="pink", pch=16)
```
